...
class ModelTopic:
    ...
    def lda_model(self, num_topics):
        ...
        lda_model = Lda(doc_term_matrix, num_topics=num_topics, id2word=dct, random_state=1,
                        chunksize=100,
                        alpha='auto',
                        eta='auto',
                        iterations=400,
        ...
        pprint(top_topics)
        """delete above until test"""

        import pyLDAvis.gensim
        # sort_topics=False, alternatively keep the same ordering as gensim
        vis = pyLDAvis.gensim.prepare(lda_model, doc_term_matrix, dct, R=15, sort_topics=True, mds='mmds')#, mds='tsne') # mds='mmds'
        # pyLDAvis.show(vis)
